...
object MultiSamples_SH {
    def main(args: Array[String]): Unit = {
        /**
         * 统计群体流量分布特征，全天和分时
         */
        val spark = SparkSession.builder().appName("MultiSamples_SH").getOrCreate()
        ...
        val allDayFlowMatrix = Array.ofDim[Float](313, 313)
        ...
        val periodFlow = odPair.map(line => (line._2, line._1))
            ...
            .mapValues(data=>{
                ...
                val matrix = Array.ofDim[Float](313,313)
                for ( p <- count.keys) {
        ...
        val features = irregularTrip.map(line => {
            ...
            for (i <- 1.to(20)) {
                ...
                val feature_i_1 = Array.ofDim[Int](313)
                val feature_i_2 = Array.ofDim[Int](313)
                val stationsNum = Array.ofDim[Int](313)
        ...
        samples.repartition(1)
            ...
            .saveAsTextFile(args(0) + "/zlt/RCB-2021/MultiSamples_SH")
