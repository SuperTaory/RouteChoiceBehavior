...
object OriginalFeature_SH {
    def main(args: Array[String]): Unit = {
        /**
         * 统计群体流量分布特征，全天和分时
         */
        val spark = SparkSession.builder().appName("OriginalFeature_SH").getOrCreate()
        ...
        val allDayFlowMatrix = Array.ofDim[Float](Num, Num)
        ...
        val periodFlow = odPair.map(line => (line._2, line._1))
            ...
            .mapValues(data=>{
                ...
                val matrix = Array.ofDim[Float](Num,Num)
                for ( p <- count.keys) {
        ...
        // 读取SH AFC数据 (2100276033,2015-04-03 15:39:56,7号线昌平路,0.00,2015-04-03 15:40:47,7号线昌平路,3.00)
        val odPairWithID = sc.textFile(args(0) + "/SH/SubwayPair/*").map(line => {
            val fields = line.split(',')
        ...
        val irregularTrip = groupByID.map(line => {
            ...
            // 存储个体固定时段的目的站点分布特征
            val feature_i_1 = Array.ofDim[Int](Num)
            // 存储个体固定起点的目的站点分布特征
            val feature_i_2 = Array.ofDim[Int](Num)
            val pairs = line._2
            val daySets = line._3
            val stationsNum = Array.ofDim[Int](Num)
